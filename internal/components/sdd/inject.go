...
func Inject(homeDir string, adapter agents.Adapter, sddMode model.SDDModeID, options ...InjectOptions) (InjectionResult, error) {
	...
	if adapter.Agent() == model.AgentOpenCode {
		...
		// OPSX: multi-mode and single-mode both use the same slim overlay
		// (only sdd-orchestrator, no sdd-apply sub-agent). Skip sdd-apply check.
	}
...
// sddOrchestratorMarkers are used to detect if SDD/OPSX content was already injected
...
var claudeModelAssignmentReasons = map[string]string{
	...
	"explore":      "Reads code, thinking partner",
