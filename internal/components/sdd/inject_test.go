...
func TestInjectClaudeCustomModelAssignments(t *testing.T) {
	...
	for _, want := range []string{
		...
		"| default | haiku | General delegation |",
	} {
...
func TestInjectVSCodeWritesSDDOrchestratorAndSkills(t *testing.T) {
	...
	text := string(content)
	// Accept both legacy SDD and new OPSX orchestrator content.
	hasOrchContent := strings.Contains(text, "Spec-Driven Development") || strings.Contains(text, "OPSX") || strings.Contains(text, "openspec")
	if !hasOrchContent {
...
func TestInjectOpenCodeMultiMode(t *testing.T) {
	...
	for _, phase := range []string{"sdd-init", "sdd-apply", "sdd-verify", "sdd-explore", "sdd-propose", "sdd-spec", "sdd-design", "sdd-tasks", "sdd-archive"} {
		if _, ok := agentMap[phase]; ok {
			t.Fatalf("OPSX overlay should NOT have legacy sub-agent %q", phase)
		}
	}
...
func TestInjectOpenCodeSubagentPromptsStayExecutorScoped(t *testing.T) {
	// OPSX: The overlay no longer includes sdd-* sub-agents.
	// This test now verifies that the orchestrator is present and no legacy sub-agents leak in.
	home := t.TempDir()
	...
	// OPSX: no legacy sdd-* sub-agents should be present.
...
func TestInjectOpenCodeEmptySDDModeDefaultsSingle(t *testing.T) {
	...
	// OPSX: slim overlay — only sdd-orchestrator, no sdd-* sub-agents.
	if _, ok := agentMap["sdd-orchestrator"]; !ok {
		t.Fatal("missing sdd-orchestrator agent")
	}
	if len(agentMap) != 1 {
		t.Fatalf("agent count = %d, want 1 (only sdd-orchestrator in OPSX)", len(agentMap))
	}
	...
	for _, subAgent := range []string{"sdd-init", "sdd-apply", "sdd-verify", "sdd-explore", "sdd-propose", "sdd-spec", "sdd-design", "sdd-tasks", "sdd-archive"} {
		if _, ok := agentMap[subAgent]; ok {
			t.Fatalf("OPSX overlay should NOT have legacy sub-agent %q", subAgent)
		}
	}
}
...
func TestInjectOpenCodeMultiModeWithModelAssignments(t *testing.T) {
	// OPSX: model assignments apply to sdd-orchestrator only (no sub-agents in overlay).
	home := t.TempDir()
	mockNoPackageManager(t)
	...
	// Verify sdd-orchestrator has the assigned model.
	orchAgent, ok := agentMap["sdd-orchestrator"].(map[string]any)
	if !ok {
		t.Fatal("sdd-orchestrator agent not found or wrong type")
	}
	if m, _ := orchAgent["model"].(string); m != "anthropic/claude-sonnet-4-20250514" {
		t.Fatalf("sdd-orchestrator model = %q, want %q", m, "anthropic/claude-sonnet-4-20250514")
	}
}
...
func TestInjectOpenCodeMultiModeNoAssignmentsNoModel(t *testing.T) {
	...
	agentMap, _ := root["agent"].(map[string]any)
	// OPSX: only sdd-orchestrator in the overlay. Without assignments, no model field.
	orchDef, ok := agentMap["sdd-orchestrator"].(map[string]any)
	if !ok {
		t.Fatal("sdd-orchestrator agent not found or wrong type")
	}
	if _, hasModel := orchDef["model"]; hasModel {
		t.Fatal("sdd-orchestrator should NOT have model field when no assignments given")
	}
}
...
func TestInjectOpenCodeMultiModeUsesRootModelForUnassignedAgents(t *testing.T) {
	...
	// OPSX: only sdd-orchestrator in overlay. It's new so should get rootModel.
	for _, phase := range []string{"sdd-orchestrator"} {
...
func TestInjectOpenCodeMultiModeExplicitAssignmentsDoNotSpread(t *testing.T) {
	...
	// OPSX: only sdd-orchestrator in overlay.
	// If sdd-orchestrator was explicitly assigned, it gets that model.
	// If not assigned but root model exists, it gets root model.
	orchAgent, ok := agentMap["sdd-orchestrator"].(map[string]any)
	if !ok {
		t.Fatal("sdd-orchestrator agent not found or wrong type")
	}
	if m, _ := orchAgent["model"].(string); m != "openai/gpt-5" {
		t.Fatalf("sdd-orchestrator model = %q, want %q (root model should apply to unassigned new agents)", m, "openai/gpt-5")
	}
}
...
func TestInjectOpenCodeSingleModeDoesNotInjectModels(t *testing.T) {
	...
	// Single mode should NOT inject model fields.
	// OPSX: sdd-orchestrator is always present, but without model in single mode.
	orchAgent, ok := agentMap["sdd-orchestrator"].(map[string]any)
	if !ok {
		t.Fatal("sdd-orchestrator agent not found or wrong type")
	}
	if _, hasModel := orchAgent["model"]; hasModel {
		t.Fatal("sdd-orchestrator should NOT have model field in single mode")
	}
...
func TestInjectOpenCodeMultiModePreservesExistingAgentModels(t *testing.T) {
  ...
  "agent": {
	...
	// OPSX: pre-existing sdd-apply in user config is preserved via deep merge.
	// sdd-orchestrator (new) should get root model.
	orchAgent, ok := agentMap["sdd-orchestrator"].(map[string]any)
	if !ok {
		t.Fatal("sdd-orchestrator agent not found or wrong type")
	}
	if m, _ := orchAgent["model"].(string); m != "openai/gpt-5" {
		t.Fatalf("sdd-orchestrator model = %q, want %q (new agent should get root model)", m, "openai/gpt-5")
	}
}
...
func TestInjectOpenCodeMultiModeExistingAgentWithNoModelIsNotTouched(t *testing.T) {
  ...
  "agent": {
	...
	// OPSX: user pre-configured sdd-apply without model — that entry is preserved.
	// sdd-orchestrator (new) should get root model.
	orchAgent, ok := agentMap["sdd-orchestrator"].(map[string]any)
	if !ok {
		t.Fatal("sdd-orchestrator agent not found or wrong type")
